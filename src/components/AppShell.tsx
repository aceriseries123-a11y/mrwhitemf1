    ...
    return (
      <div className="flex h-7 items-center border-b border-border bg-surface/60 px-4 gap-2">
        ...
        <span className="font-mono text-[10px] uppercase tracking-wider text-muted-foreground">
          Market data unavailable · Yahoo Finance
        </span>
      </div>
    );
  }
...
export function AppShell({ children, title }: { children: ReactNode; title?: string }) {
  ...
  return (
    <div className="min-h-screen bg-background bg-grid">
      ...
      {/* Sidebar */}
      <aside
        className={`fixed inset-y-0 left-0 z-40 flex w-60 flex-col border-r border-border bg-surface/95 backdrop-blur-xl transition-transform duration-200 ${
          open ? "translate-x-0" : "-translate-x-full"
        } lg:translate-x-0`}
      >
        {/* Logo */}
        <div className="flex h-14 shrink-0 items-center gap-2.5 border-b border-border px-4">
          <div className="grid h-8 w-8 place-items-center rounded-xl bg-gradient-to-br from-primary to-cyan shadow-glow">
            <Activity className="h-4 w-4 text-primary-foreground" />
          </div>
          <span className="font-display text-sm font-bold tracking-tight">
            QUANTFUND<span className="text-cyan">.</span>
          </span>
          <button
            className="ml-auto rounded-lg border border-border p-1 text-muted-foreground hover:text-foreground lg:hidden"
            onClick={() => setOpen(false)}
        ...
        <div className="shrink-0 border-t border-border px-3 py-3">
          <div className="rounded-xl border border-border bg-card/60 p-3 text-xs">
            <div className="mb-2 font-mono text-[9px] uppercase tracking-wider text-muted-foreground">Data sources</div>
            <div className="space-y-1.5">
              <div className="flex items-center gap-2">
                <span className="h-1.5 w-1.5 animate-pulse rounded-full bg-positive" />
                <span className="text-foreground">AMFI · mfapi.in</span>
              </div>
              <div className="flex items-center gap-2">
                <span className="h-1.5 w-1.5 animate-pulse rounded-full bg-positive" />
      ...
      <div className="lg:pl-60">
        ...
        <header className="sticky top-0 z-30 flex h-14 items-center gap-3 border-b border-border bg-background/80 px-4 backdrop-blur-xl">
          <button
            className="grid h-8 w-8 place-items-center rounded-lg border border-border bg-surface text-muted-foreground hover:text-foreground lg:hidden"
          ...
          <h1 className="font-display text-sm font-semibold">{title ?? "Dashboard"}</h1>

          <div className="ml-auto hidden items-center gap-2 md:flex">
            <Link
              to="/explorer"
              className="flex items-center gap-2 rounded-lg border border-border bg-surface px-3 py-1.5 text-xs text-muted-foreground transition-colors hover:text-foreground"
            >
          ...
          <button
            className="grid h-8 w-8 place-items-center rounded-lg border border-border bg-surface text-muted-foreground hover:text-foreground"
          ...
          <div
            className="grid h-8 w-8 place-items-center rounded-full bg-gradient-to-br from-primary to-cyan font-mono text-xs font-bold text-primary-foreground"
            aria-hidden="true"
          >
            QF
          </div>
